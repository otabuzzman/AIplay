...
extension NetworkView {
    class NetworkViewModel: ObservableObject {
        ...
        func query(subset: MNISTSubset.Purpose = .test, count: Int = 0) async -> Float {
            let n = dataset.count(in: subset)
            let samples = count == 0 ? n : min(count, n)
        ...
        func loss(image index: Int) -> Float {
            let (image, label) = dataset.fetch(index, from: .test)
            let (loss, _) = nnxd.network.loss(for: image.toInput(), with: label.toTarget())
            return loss
        }
